...
![Authentication methods policy showing FIDO2, Authenticator, and TAP enabled with SMS and Voice disabled](../screenshots/27a-auth-methods-policy-before.png)
*Authentication methods policy. Phishing-resistant and passwordless methods enabled; SMS and Voice call disabled.*
...
The User registration details report was captured as the pre-enforcement baseline.

![User registration details showing three capable accounts and all workforce users Not-Capable](../screenshots/27-auth-methods-registration-details-before.png)
*Pre-enforcement baseline. Only the primary admin and two break-glass accounts are MFA-capable. All 25 workforce users are Not-Capable across every method column.*
...
![CA01 block legacy authentication policy details in report-only](../screenshots/28-ca01-blocklegacyauth-details.png)
*CA01 blocks legacy authentication. Legacy protocols bypass MFA entirely, so this is one of the highest-value policies on the board.*
...
![CA03 policy build blade with Register security information user action selected](../screenshots/30-ca03-registration-target-useraction.png)
*CA03 targets the Register security information user action, not a cloud app. This is the step most people configure incorrectly.*
...
![CA04 renamed from CA11 into the global block](../screenshots/29b-ca04-renamed-from-ca11.png)
*Numbering correction. The all-users Azure management policy was renamed from CA11 to CA04, moving it into the global block where it belongs.*

With all policies built, the full persona board sits in report-only:

![Full five-policy Conditional Access board, all in report-only](../screenshots/31-ca03-report-only-created.png)
*The complete persona board. Five user-created policies, zero Microsoft-managed, all report-only.*
...
![Named locations list showing Chicago trusted and High-risk countries](../screenshots/32-named-locations-both-chicago-trusted.png)
*Both named locations. Chicago is marked Trusted; neither is referenced by a policy yet.*
...
![High-risk countries named location](../screenshots/33-named-location-highrisk-countries.png)
*The high-risk countries location, built as a block candidate for a later policy.*
...
![Tom Gallagher sign-in report-only tab showing CA02 Success](../screenshots/35-signin-report-only-verdicts-tom.png)
*Tom signed in with a TAP. CA02 returned Success, because a Temporary Access Pass satisfies an MFA requirement.*
...
During this sign-in, David was pushed into the Authenticator registration flow:

![David Okonkwo pushed into Microsoft Authenticator registration](../screenshots/36a-david-registration-prompt.png)
*A password-only user with no registered method is driven into combined registration.*

The report-only verdict tells the real story:

![David Okonkwo sign-in report-only tab showing CA02 User action required](../screenshots/36-signin-report-only-useraction-david-passwordonly.png)
*CA02 returned User action required for the password-only sign-in. This is the enforcement-gap proof.*

CA02 returned "User action required." At real cutover, a password-only user with no registered method would be interrupted and required to complete MFA they cannot yet do, and would be blocked.
...
Security Defaults was confirmed Disabled on the tenant Properties page.

![Tenant Properties confirming Security Defaults is Disabled](../screenshots/37-security-defaults-disabled-confirmed.png)
*Security Defaults confirmed Disabled. This clears the mutual-exclusivity blocker for cutover. The personal technical-contact email is masked.*

This clears the mutual-exclusivity blocker for cutover, and it also means the tenant currently has no active MFA enforcement, since all policies remain in report-only. That unprotected window is itself the argument for proceeding to enforcement.
